...
from fxns.hmm_related import forward_backward,viterbi,result_consensus_bayesian_hmm,initialize_tmatrix
...
def consensus_vb_em_hmm(x,nstates,maxiters=1000,threshold=1e-10,prior_strengths=None):
	...
	result = result_consensus_bayesian_hmm(r,a,b,m,beta,pi,tmatrix,E_lnlam,E_lnpi,E_lntm,likelihood[:iteration+1],iteration)
	result.viterbi = []
	for i in range(len(x)):
		result.viterbi.append(viterbi(x[i],result.mu,result.var,result.tmatrix,result.ppi))
...
def consensus_vb_em_hmm_parallel(x,nstates,maxiters=1000,threshold=1e-10,nrestarts=1,prior_strengths=None,ncpu=1):
	...
	if platform != 'win32' and ncpu != 1 and nrestarts != 1:
		...
		results = [pool.apply_async(consensus_vb_em_hmm, args=(x,nstates,maxiters,threshold,prior_strengths)) for i in xrange(nrestarts)]
		results = [p.get() for p in results]
		pool.close()
	else:
		results = [consensus_vb_em_hmm(x,nstates,maxiters,threshold,prior_strengths) for i in xrange(nrestarts)]
